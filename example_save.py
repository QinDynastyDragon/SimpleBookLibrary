...
def save_persons(filename, persons):
	...
	with open(filename, 'w', encoding='utf8') as f:
		# Turn each person into a string, then s'''ave one string each line
		for p in persons:
...
def load_persons(filename):
	'''Load a list of Person from a file containing one string representing the Person each line.'''
	persons = []
	with open(filename, 'r', encoding='utf8') as f:
		for line in f:  # Loop each line
			# NOTE: How to turn each line into a Person? How do we know each part name, and which part is age?
			...
			# So now: list = ['Example Name', 'example age']
			name = line[0]
			age = int(line[1])

			# The following two lines are stupid.
